    ...
    <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
      ...
      <div class="md:col-span-12">
        ...
        <textarea name="descripcion" rows="4"
                  class="mt-2 w-full rounded-xl border border-slate-900/10 px-3 py-3 bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 outline-none"
                  placeholder="Descripción general del proyecto...">{{ old('descripcion', $proyecto->descripcion) }}</textarea>
      </div>
      ...
      <div class="md:col-span-6">
        ...
        <select name="responsable_id"
          ...
          @foreach($usuarios as $u)
            <option value="{{ $u->id }}" @selected((string)old('responsable_id', $proyecto->responsable_id) === (string)$u->id)>
              {{ $u->name }}
            </option>
          @endforeach
        </select>
