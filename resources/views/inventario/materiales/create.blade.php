...
<div class="max-w-4xl mx-auto">
  ...
  <div class="flex flex-wrap items-start justify-between gap-3">
    <div>
      <h1 class="text-xl font-extrabold tracking-tight">Nuevo Material</h1>
      <p class="text-sm text-slate-500 mt-1">Registra un material en tu empresa.</p>
    </div>
  ...
  @if(!$hasUnidades)
    <div class="mt-4 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-red-800 flex items-center gap-2">
      ...
      <span>No hay unidades registradas. Debes crear al menos una unidad antes de crear materiales.</span>
    </div>
  @endif
  ...
  <div class="mt-5 rounded-2xl border border-slate-900/10 bg-white shadow-sm">
    <form method="POST" action="{{ route('inventario.materiales.store') }}" class="p-5">
      @csrf
          ...
          <select class="mt-2 w-full h-11 rounded-xl border border-slate-900/10 px-3
            ...
            @if($hasUnidades)
              @foreach($unidades as $u)
                <option value="{{ $u->id }}" @selected((string)old('unidad_id')===(string)$u->id)>
